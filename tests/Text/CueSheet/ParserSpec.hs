...
spec =
  describe "parseCueSheet" $ do
    ...
    it "at least one track should be declared" $ do
      ...
      parseCueSheet "" bs `shouldFailWith`
        err 109 (ueof <> etoks "REM" <> etoks "TRACK")
    it "at least one non-zero index should be declared" $ do
      ...
      parseCueSheet "" bs `shouldFailWith`
        errFancy 174 (cstm (Eec (Just 1) e))
    ...
    it "rejects invalid catalog number" $ do
      ...
      parseCueSheet "" bs `shouldFailWith` errFancy 8
    ...
    it "rejects invalid CUE text" $ do
      ...
      parseCueSheet "" bs `shouldFailWith` errFancy 10
    ...
    it "detects and reports tracks that appear out of order" $ do
      ...
      parseCueSheet "" bs `shouldFailWith` errFancy 182
    ...
    it "rejects invalid ISRC values" $ do
      ...
      parseCueSheet "" bs `shouldFailWith` errFancy 161
    ...
    it "rejects invalid number of seconds" $ do
      ...
      parseCueSheet "" bs `shouldFailWith` errFancy 168
    ...
    it "rejects invalid number of frames" $ do
      ...
      parseCueSheet "" bs `shouldFailWith` errFancy 171
    ...
    it "detects and reports indices that appear out of order" $ do
      ...
      parseCueSheet "" bs `shouldFailWith` errFancy 162
    ...
    it "rejects duplicate CATALOG command" $ do
      ...
      let es = utoks "CATA" <> foldMap etoks
            [ "CDTEXTFILE"
            , "FILE"
            , "PERFORMER"
            , "REM"
    ...
    it "rejects duplicate CDTEXTFILE command" $ do
      ...
      let es = utoks "CDTE" <> foldMap etoks
            [ "CATALOG"
            , "FILE"
            , "PERFORMER"
            , "REM"
    ...
    it "rejects duplicate PERFORMER command" $ do
      ...
      let es = utoks "PERF" <> foldMap etoks
            [ "CATALOG"
            , "CDTEXTFILE"
            , "FILE"
            , "REM"
    ...
    it "rejects duplicate TITLE command" $ do
      ...
      let es = utoks "TITL" <> foldMap etoks
            [ "CATALOG"
            , "CDTEXTFILE"
            , "FILE"
            , "PERFORMER"
    ...
    it "rejects duplicate SONGWRITER command" $ do
      ...
      let es = utoks "SONG" <> foldMap etoks
            [ "CATALOG"
            , "CDTEXTFILE"
            , "FILE"
            , "PERFORMER"
    ...
    it "rejects duplicate FLAGS command (in track)" $ do
      ...
      let es = wrappedTrivial $ utoks "FLAGS" <> foldMap etoks
            [ "INDEX"
            , "ISRC"
            , "PERFORMER"
            , "PREGAP"
    ...
    it "rejects duplicate PREGAP command (in track)" $ do
      ...
      let es = wrappedTrivial $ utoks "PREGA" <> foldMap etoks
            ...
            , "TITLE" ]
      parseCueSheet "" bs `shouldFailWith`
        errFancy 176 (cstm (Eec (Just 1) es))
